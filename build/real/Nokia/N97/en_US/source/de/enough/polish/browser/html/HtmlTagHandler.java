...
//				}
			...
			if (opening) {
				...
				if (style == null) {
					...
					//#style browserLink
					UiAccess.setStyle(container, de.enough.polish.ui.StyleSheet.browserlinkStyle );
				}
			} else {
...
//				System.out.println("closing <a>: container.size()=" + container.size() + ", linkItem=" + linkItem + ", style=" + (contStyle != null ? contStyle.name : "<no style>") );
				if (this.anchorHref != null) {
					if (contStyle != null) {
						...
						//#style browserLink
						UiAccess.setStyle( linkItem , de.enough.polish.ui.StyleSheet.browserlinkStyle );
					}
			...
			else if (TAG_INPUT.equals(tagName))
			{
				if (this.currentForm != null)
				{
					...
					if (INPUTTYPE_TEXT.equals(type) || INPUTTYPE_PASSWORD.equals(type) || INPUTTYPE_NUMERIC.equals(type))
					{
						...
						//#style browserInput
						TextField textField = new TextField((String)attributeMap.get("label"), value, 100, constraints, de.enough.polish.ui.StyleSheet.browserinputStyle );
						if (style != null) {
							textField.setStyle(style);
						}
						add(textField);
					...
					if (INPUTTYPE_SUBMIT.equals(type))
					{
						...
						//#style browserLink
						StringItem buttonItem = new StringItem((String)attributeMap.get("label"), value, de.enough.polish.ui.StyleSheet.browserlinkStyle );
						if (style != null) {
							buttonItem.setStyle(style);
						}
						buttonItem.setDefaultCommand(CMD_SUBMIT);
						...
						if (choiceGroup == null || !name.equals(choiceGroup.getAttribute(ATTR_NAME))) {
							...
							if (isCheckBox) {
								...
								choiceGroup = new ChoiceGroup(groupLabel,  Choice.MULTIPLE , de.enough.polish.ui.StyleSheet.browserchoicegroupmultipleStyle );
							} else {
								String choiceType = (String) attributeMap.get("choice");
								if ("popup".equals(choiceType)) {
									//#style browserChoiceGroupPopup
									choiceGroup = new ChoiceGroup(groupLabel,  Choice.POPUP , de.enough.polish.ui.StyleSheet.browserchoicegrouppopupStyle );								
									this.currentRadioChoiceGroup = choiceGroup;
								} else {
									//#style browserChoiceGroupExclusive
									choiceGroup = new ChoiceGroup(groupLabel,  Choice.EXCLUSIVE , de.enough.polish.ui.StyleSheet.browserchoicegroupexclusiveStyle );
									this.currentRadioChoiceGroup = choiceGroup;
								}								
							}
						...
						if (isCheckBox) {
							...
							item = new ChoiceItem(label, null, Choice.MULTIPLE, de.enough.polish.ui.StyleSheet.browsercheckboxStyle );
						} else {
							//#style browserRadio
							item = new ChoiceItem(label, null, choiceGroup.getType() , de.enough.polish.ui.StyleSheet.browserradioStyle );							
						}
				...
				//#if polish.css.style.browsertextcode && !polish.LibraryBuild
					...
					this.textStyle = (de.enough.polish.ui.StyleSheet.browsertextcodeStyle );
				//#endif
			}
		}
		else 
